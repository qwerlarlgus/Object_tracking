...
contours3, _ = cv2.findContours(src_bin, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
#print(len(contours1))
#print(len(contours2))
print(len(contours3))

for pts in contours3:
    if cv2.contourArea(pts) < 1000:
        continue
    
    for i in range(len(contours3)):
        cv2.drawContours(dst3, contours3, i, (0, 255,0), 1)
'''
    approx = cv2.approxPolyDP(pts, cv2.arcLength(pts, True)*0.02, True)
    if len(approx) !=4:
        continue
    cv2.polylines(src_bin, pts, True, (0, 0, 255))
